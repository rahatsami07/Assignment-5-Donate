...
.addEventListener('click', function(){
    const inputDonate = getInputValueById('feni-input-donate');
    if(isNaN(inputDonate)){
        alert('Failed to Donate');
        hideModal('my_modal_2');
    ...
    // addding process
    const donateAmount = getTextAmountById('feni-donate-amount');

    const donateBalance = inputDonate + donateAmount;


    document.getElementById('feni-donate-amount').innerText = donateBalance;
