...
.addEventListener('click', function(){
    ...
    const inputDonate = getInputValueById('input-donate');
    if(isNaN(inputDonate)){
        alert('Failed to Donate');
        hideModal('my_modal_1');
    ...
    // addding process
    const donateAmount = getTextAmountById('donate-amount');

    const donateBalance = inputDonate + donateAmount;


    document.getElementById('donate-amount').innerText = donateBalance;
